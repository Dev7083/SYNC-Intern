...
def Download_vid():
    ...
    if choice == choices[0] or choices[2]:
        ...
        try:
            get = messagebox.askyesno(
                "Do You Want To Download", f"File Size: {round(size* 0.000001, 2)} MB"
            )
    ...
    if choice == choices[1] or choices[2] :
        filenamee = slugify(YouTube(url).title)
        audio = (
            yt.streams.filter(only_audio=True)
            .last()
            .download(Foldername, filenamee + ".webm")
        )
...
saveEntry = Button(
    root, width=13, bg="red", fg="white", text="Choose Directory:", command=openloc
)
saveEntry.grid()

locationnerror = Label(
    root, text="Directory causes Error", fg="red", font=("bahnschrift semilight", 13)
)
locationnerror.grid()
